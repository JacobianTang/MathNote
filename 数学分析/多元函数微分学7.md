...
对于这样的点，函数$f$的增量，可以用函数Ｆ的增量来代替，但是不同的是将式中的一切$\lambda_i$替换成$\lambda_i^{0}$,此时
...
...\\
\Delta x_{n+m}=dx_{n+m} + \sum_{i=1}^{n}\beta_{m,i}dx_i    
\end{cases} \tag{24}$$
...
\end{cases} \tag{27}
$$
(27)是实际上约束方程所确定的曲面$S$上一点,也是曲面Ｓ在这个邻域里的参数方程．我们知道$M_0(x_1^{0},...,x_n^{0},x_{n+1}^{0},...,x_{n+m}^{0})$也在曲面Ｓ上，并且也是满足（２７）所确定的方程的．
由（２７）式，可以得到曲面Ｓ在点$M_0$处的切平面$T_s$，用(27)式所确定的切映射来确定
$$
\begin{cases}
x_1-x_1^{0}=x_1-x_1^{0}\\
...\\
...
<!-- 所以$d = (x_1-x_1^{0},...,x_n-x_n^{0},x_{n+1}-x_{n+1}^{0},...,x_{n+m}-x_{n+m}^{0})^T$实际上切平面上的一个向量．即有$d \in T_s$.

我们通过(28)式我们实际上知道，即将(28）式的右边代入左边会有，$d=(dx_1,dx_2,...,dx_n,dx_{n+1},...,dx_{n+m}) \in T_s$


也就是意味着在点$M_0$附近的微分，可以映射到$M_0$处的切平面上取． -->
