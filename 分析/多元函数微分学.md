...
[**推论可微函数的方向导数**]如果函数$f(x,y)$在点$(x_0,y_0)$处是可微的,那么沿着方向$e=(cos\alpha,sin\alpha)$的方向导数存在,且有$\frac{\partial f}{\partial e}=f_x'(x_0,y_0)cos\alpha + f_y'(x_0,y_0)sin\alpha$.
...
有时候也将f的jacobi矩阵记为$grad f(\nabla f)$即$grad f(x)=Jf(x)$称为函数f的梯度.但是这个确实有一些歧义,不同的教科书上,特别是优化的书上对于梯度和jacobi的关系的处理是不一致的.有些认为$\nabla f=Jf(x)^{T}$,虽然没有本质区别,但是需要注意.
