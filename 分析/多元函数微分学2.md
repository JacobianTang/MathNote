...
## 多元函数中值定理
[**多元函数微分中值定理**]
设定义在凸区域（连通的开集）$D \subset R^n$上的函数f可微，那么对于任何两点$a,b \in D$在由a与b确定的线段上存在一点$\xi$使得$f(b)-f(a)=Jf(\xi)(b-a)$.
...
不妨记$h=b-a$
而$\varphi'(t)=\sum_{i=1}^{n}\frac{\partial f}{\partial x_i}(a+t(b-a))(b-a)_i=J_xf(a+t(b-a))(b-a)$
...
可见结论是成立的。
不妨记$b=(b_1,...,b_n)^T,a=(a_1,...,a_n)^T$则有
$f(b)-f(a)=\sum_{i=1}^{n}\frac{\partial f}{\partial x_i}(\xi)(b_i-a_i)$
...
[**定理2**]设$a,a+h \in R^m$中的两个点，D是包含$[a,a+h]$的一个开集。如果函数$f(x)=f(x_1,x_2,...,x_m)$在D上连续可微，那么就有$f(a+h)-f(a)=\sum_{i=1}^{m}(\int_{0}^{1}\frac{\partial f}{\partial x_i}(a+th)dt)h_i$
proof:
