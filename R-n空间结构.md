...
proof:
考察点列$\{x_n\}$各项的坐标表示$x=(x_n^1,x_n^2,...,x_n^m)$
因为$\{x_n\}$有界，意味着$\exists L >0,||x_n|| \le L,n=1,2,3...$
因为$|x_n^i| \le ||x_n|| \le L,n=1,2,...$
可见$\{x_n^i\},i=1,2,...,m$也是一列有界的列．

不妨从$\{x_n^1\}$选出一列收敛的子列$\{x_{n_{1,k}}^1\}$
基于同样的序号，我们考察$\{x_{n_{1,k}}^2\}$很显然它任然是实数的一个有界序列，断定，可以从里面挑选出收敛的子列$\{x_{n_{2,k}}^2\}$,以此类推，直到最后，可以从实数的有界序列$\{x_{n_{m-1,k}}^m\}$之中，挑选出收敛的子列$\{x_{n_{m,k}}^m\}$
我们记$n_k =n_{m,k}$
$x_{n_k}=(x_{n_k}^1,x_{n_k}^2,...,x_{n_k}^m)$
那么子列$\{x_{n_k}\}$是收敛子列．
