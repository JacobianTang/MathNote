...
记$F(x_0) = \sum_{k=0}^{n}\frac{f^{(k)}(x_0)}{k!}(x-x_0)^k$
那么$R_n(x) = F(x) -F(x_0)$


[**定理**]
设函数$f(x)$在$x_0$邻域Ｉ内任意阶可导，那么在该邻域ｆ(x)可以展开为幂级数，即$f(x)=\sum_{n=0}^{\infty}\frac{f^{(n)}(x_0)}{n!}(x-x_0)^n$的充分必要条件是$f(x)$的taylor公式中的余项$R_n(x)$满足$\lim\limits_{n \to \infty}R_n(x) = 0$.
...
$f(x)=e^x,f^{(n)}(x)=e^{x}$那么任意给定$x \in (-\infty,+\infty)$
...
实际上对于幂级数$\sum_{n=0}^{\infty} \frac{x^n}{n!}$其收敛半径是$R=1/\rho,\rho = \lim\limits_{n \to \infty}\frac{|a_{n+1}|}{|a_n|}$